...
public class GameView extends AbstractView {
	...
	public GameView(Context context, AttributeSet attrs) {
		...
		this.typeface = Typeface.createFromAsset(this.getResources()
				.getAssets(), "cooper.ttf");
	...
	public void setNowScoreAnimating(boolean animating) {
		...
		this.nowScoreTextAlpha = new Value(0xff).add(0xff, 3).add(0xff, 8)
				.add(0x7f, 5);
		this.nowBonusTextScale = new Value(0).add(1, 3).add(1, 8).add(1, 40);
		this.nowBonusTextAlpha = new Value(0xff).add(0xff, 3).add(0xff, 8)
				.add(0, 10);
		this.nowRemarkTextScale = new Value(0).add(0, 3).add(0, 8).add(1, 3)
				.add(1, 8).add(1, 5);
		this.nowRemarkTextAlpha = new Value(0xff).add(0xff, 3).add(0xff, 8)
				.add(0xff, 3).add(0xff, 8).add(0, 5);
	...
	private void setReadyGoAnimating(boolean animating) {
		...
		this.nowReadyTextAlpha = new Value(0).add(0xff, 3).add(0xff, 20)
				.add(0, 10);
		this.nowGoTextAlpha = new Value(0).add(0, 3).add(0, 20).add(0xff, 3)
				.add(0xff, 20).add(0, 3);
	...
	public boolean isViewAnimating() {
		return isNowScoreAnimating || isNowRemarkAnimating
				|| isReadyGoAnimating || isViewLocked();
	}
	...
	public void setScoreTextByCenter(int scoreHeight, String scoreText,
			String bonusText, String remarkText, float x, float y) {
		...
		int textLength = (int) Math.max(scorePaint.measureText(nowScoreText),
				scorePaint.measureText(remarkText));
		textLength = (int) Math.max(textLength,
				bonusPaint.measureText(bonusText));
		if (bonusText.length() > 0) {
			Point nowMiddlePoint = fixPointByCenter((int) x, (int) (y),
					textLength, nowScoreTextHeight + nowBonusTextHeight);
			nowScoreTextPoint.set(nowMiddlePoint.x, nowMiddlePoint.y);
			nowScoreTextPoint.y = (int) (nowScoreTextPoint.y
					- nowScoreTextHeight / 2 - (scoreFM.ascent + scoreFM.descent) / 2);
			nowBonusTextPoint.set(nowMiddlePoint.x, nowMiddlePoint.y);
			nowBonusTextPoint.y = (int) (nowBonusTextPoint.y
					+ nowBonusTextHeight / 2 - (bonusFM.ascent + bonusFM.descent) / 2);
			nowScoreTextDeltaPoint.set((int) (this.getWidth()
					- nowScoreTextPoint.x - scorePaint
					.measureText(nowScoreText) / 3) / 10,
					-nowScoreTextPoint.y / 10);
			nowRemarkTextPoint.set(nowScoreTextPoint.x, nowScoreTextPoint.y);
		} else {
			Point nowMiddlePoint = fixPointByCenter((int) x, (int) (y),
					textLength, nowScoreTextHeight);
			nowScoreTextPoint.set(nowMiddlePoint.x, nowMiddlePoint.y);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		// ItemBlur
		Bitmap tmpBitmap;
		tmpBitmap = ((BitmapDrawable) getResources().getDrawable(
				R.drawable.cblur)).getBitmap();
		...
		for (int i = 0; i < COLORS_SIZE; ++i) {
			tmpBitmap = ((BitmapDrawable) getResources().getDrawable(
					R.drawable.c1 + i)).getBitmap();
			bitItemsColorImage[i] = createDoubleBitmapByScale(tmpBitmap,
					(float) itemImageWidth / tmpBitmap.getWidth(),
					(float) itemImageHeight / tmpBitmap.getHeight());
		}
		// ScoreTextColor
		...
		tmpBitmap = ((BitmapDrawable) getResources().getDrawable(
				R.drawable.cblur)).getBitmap();
		bitItemBlur = createDoubleBitmapByScale(tmpBitmap,
				(float) itemImageWidth / tmpBitmap.getWidth(),
				(float) itemImageHeight / tmpBitmap.getHeight());
		...
		for (int i = 0; i < FACES_SIZE; ++i) {
			...
			tmpBitmap = ((BitmapDrawable) getResources().getDrawable(
					R.drawable.ea1 + i)).getBitmap();
			bitItemsFaceImage[i].image = createDoubleBitmapByScale(tmpBitmap,
					(float) itemImageWidth / tmpBitmap.getWidth(),
					(float) itemImageHeight / tmpBitmap.getHeight());
			tmpBitmap = ((BitmapDrawable) getResources().getDrawable(
					R.drawable.eo1 + i)).getBitmap();
			bitItemsFaceImage[i].imageAnother = createDoubleBitmapByScale(
					tmpBitmap, (float) itemImageWidth / tmpBitmap.getWidth(),
					(float) itemImageHeight / tmpBitmap.getHeight());
		}
		// ItemImageBlur
		tmpBitmap = ((BitmapDrawable) getResources()
				.getDrawable(R.drawable.eaf)).getBitmap();
		bitItemImageBlur = createDoubleBitmapByScale(tmpBitmap,
				(float) itemImageWidth / tmpBitmap.getWidth(),
				(float) itemImageHeight / tmpBitmap.getHeight());
		// Warning
		bitWarning = ((BitmapDrawable) getResources().getDrawable(
				R.drawable.warning)).getBitmap();
		bitWarning = createBitmapByScale(bitWarning, itemImageHeight * 2
				/ bitWarning.getHeight(),
				itemImageHeight * 2 / bitWarning.getHeight());
		// Laser
		bitLaser = ((BitmapDrawable) getResources().getDrawable(
				R.drawable.laser)).getBitmap();
		// Background
		int backgroundID = 0;
		...
		bitBackgroundImage = ((BitmapDrawable) getResources().getDrawable(
				backgroundID)).getBitmap();
		// --------------------------------Get Picture
		// END----------------------------//
	...
	protected void onDraw(Canvas canvas) {
		...
		drawImageByCorner(canvas, bitBackgroundImage, 0, 0,
				bitBackgroundImage.getWidth(), bitBackgroundImage.getHeight(),
				0, 0, this.getWidth(), this.getHeight(), alphaPaint);
		// FillMode.Background
		for (int i = 0; i < game.getColumnsSize(); ++i)
			for (int j = 0; j < game.getRowsSize(); ++j)
				if (!game.getPositionFilled(i, j)) {
					alphaPaint.setAlpha(0xFF);
					canvas.drawRect(i * itemImageWidth, j * itemImageHeight,
							(i + 1) * itemImageWidth,
							(j + 1) * itemImageHeight, alphaPaint);
				}
		...
		for (int i = 0; i < game.getColumnsSize(); ++i)
			for (int j = 0; j < game.getRowsSize(); ++j) {
				if (game.getItemColor(i, j) != 0
						&& (!game.getItemAffiliated(i, j) || game.getItemLarge(
								i, j))) {
					int k = game.getItemLarge(i, j) ? DoubleBitmap.LARGE
							: DoubleBitmap.SMALL;
					int x = game.getItemLarge(i, j) ? (int) ((i + 1) * itemImageWidth)
							: (int) ((i + 0.5) * itemImageWidth);
					int y = game.getItemLarge(i, j) ? (int) ((j + 1) * itemImageHeight)
							: (int) ((j + 0.5) * itemImageHeight);
					if (isItemFading && game.getItemFocus(i, j)) {
						nowScoreTextColor = scoreTextColors[game.getItemColor(
								i, j) - 1];
						if (itemFadingImageScale.getDouble() > EPSILON) {
							Bitmap tmpBitmap = createBitmapByScale(
									bitItemsColorImage[game.getItemColor(i, j) - 1]
											.getBitmap(k), itemFadingImageScale
											.getDouble(), itemFadingImageScale
											.getDouble());
							drawImageByCenter(canvas, tmpBitmap, 0, 0,
									tmpBitmap.getWidth(),
									tmpBitmap.getHeight(), x, y,
									tmpBitmap.getWidth(),
									tmpBitmap.getHeight(), null);
						}
						...
						if (itemAppearingImageScale.getDouble() > EPSILON) {
							Bitmap tmpBitmap = createBitmapByScale(
									bitItemsColorImage[game.getItemColor(i, j) - 1]
											.getBitmap(k),
									itemAppearingImageScale.getDouble(),
									itemAppearingImageScale.getDouble());
							drawImageByCenter(canvas, tmpBitmap, 0, 0,
									tmpBitmap.getWidth(),
									tmpBitmap.getHeight(), x, y,
						...
						canvas.drawBitmap(bitItemsColorImage[game.getItemColor(
								i, j) - 1].getBitmap(k), i * itemImageWidth, j
								* itemImageHeight, null);
						if (game.getItemFocus(i, j)) {
							alphaPaint.setAlpha(0x7F);
							canvas.drawBitmap(bitItemImageBlur.getBitmap(k), i
									* itemImageWidth, j * itemImageHeight, null);
							canvas.drawBitmap(bitItemBlur.getBitmap(k), i
									* itemImageWidth, j * itemImageHeight,
									alphaPaint);
						} else if (game.getItemFaceChanged(i, j))
							canvas.drawBitmap(bitItemsFaceImage[game
									.getItemFace(i, j)].imageAnother
									.getBitmap(k), i * itemImageWidth, j
									* itemImageHeight, null);
						else
							canvas.drawBitmap(bitItemsFaceImage[game
									.getItemFace(i, j)].image.getBitmap(k), i
									* itemImageWidth, j * itemImageHeight, null);
					}
				}
			}
		...
		if (game.getClass().equals(TimeMode.class)) {
			...
			drawImageByCenter(canvas, bitLaser, 0, 0, bitLaser.getWidth(),
					bitLaser.getHeight(), getWidth() / 2, getHeight() / 2,
					getWidth(), (int) itemImageHeight * 2, alphaPaint);
		}
		// Value
		if (this.isNowScoreAnimating) {
			...
			int textSize = (int) (this.nowScoreTextHeight * this.nowScoreTextScale
					.getDouble());
			int alpha = this.nowScoreTextAlpha.getInt();
			if (alpha < 0xff)
				this.nowScoreTextPoint.offset(nowScoreTextDeltaPoint.x,
						nowScoreTextDeltaPoint.y);
			if (textSize > 0)
				drawTextByBaseline(canvas, this.nowScoreText,
						this.nowScoreTextPoint, textSize, nowScoreTextColor,
						Color.WHITE, alpha, textPaint);
			this.nowBonusTextScale.next();
			this.nowBonusTextAlpha.next();
			textSize = (int) (this.nowBonusTextHeight * this.nowBonusTextScale
					.getDouble());
			alpha = this.nowBonusTextAlpha.getInt();
			if (textSize > 0)
				drawTextByBaseline(canvas, this.nowBonusText,
						this.nowBonusTextPoint, textSize, nowScoreTextColor,
						Color.WHITE, alpha, textPaint);
		}
		if (this.isNowRemarkAnimating) {
			...
			int textSize = (int) (this.nowScoreTextHeight * this.nowRemarkTextScale
					.getDouble());
			int alpha = this.nowRemarkTextAlpha.getInt();
			if (textSize > 0)
				drawTextByBaseline(canvas, this.nowRemarkText,
						this.nowRemarkTextPoint, textSize, nowScoreTextColor,
						Color.WHITE, alpha, textPaint);
		}
		if (this.isWarning) {
			...
			drawImageByCenter(canvas, bitWarning, 0, 0, bitWarning.getWidth(),
					bitWarning.getHeight(), getWidth() / 2, getHeight() / 2,
					(int) itemImageWidth * 2, (int) itemImageHeight * 2,
					alphaPaint);
		}
		// Ready & Go
		if (isReadyGoAnimating) {
			if (nowGoTextAlpha != null && nowReadyTextAlpha != null) {
				if (nowGoTextAlpha.next())
					drawTextByBaseline(canvas,
							getResources().getString(R.string.text_go),
							nowGoTextPoint, getHeight() / 5, Color.WHITE,
							Color.BLACK, nowGoTextAlpha.getInt(), textPaint);
				else {
					game.startGame();
					isReadyGoAnimating = false;
				}
				if (nowGoTextAlpha.getInt() > 0)
					nowReadyTextPoint.offset(nowReadyTextDeltaPoint.x,
							nowReadyTextDeltaPoint.y);
				if (nowReadyTextAlpha.next())
					drawTextByBaseline(canvas, nowReadyText, nowReadyTextPoint,
							getHeight() / 10, Color.WHITE, Color.BLACK,
							nowReadyTextAlpha.getInt(), textPaint);
			}
		}
	}
